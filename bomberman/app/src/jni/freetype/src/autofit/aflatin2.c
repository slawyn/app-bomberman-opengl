...
#define AF_LATIN_MAX_TEST_CHARACTERS  12
                               ...
                               FT_Face          face )
  {
    ...
    /* 'af_latin2_blue_chars[blues]' string, then compute its mUp-most or */
    /* mDown-most points (depending on `AF_IS_TOP_BLUE')                */
    ...
    for ( bb = 0; bb < AF_LATIN_BLUE_MAX; bb++ )
    {
      ...
      /* there are sometimes problems: if the overshoot position of mUp     */
      /* zones is under its reference position, or the opposite for mDown  */
       ...
       * in order to optimize the pixel grid alignment of the mUp of small
       * letters.
       */
                              ...
                              AF_Dimension     dim )
  {
    ...
    /*
     * correct Y scale to optimize the alignment of the mUp of small
    ...
    if ( dim == AF_DIMENSION_VERT )
    {
      ...
      if ( blue )
      {
        ...
        if ( scaled != fitted )
        {
          ...
          AF_LOG(( "== scaled x-mUp = %.2g  fitted = %.2g, scaling = %.4g\n", scaled/64.0, fitted/64.0, (fitted*1.0)/scaled ));
        }
#endif
      }
    }
...
#define  SORT_SEGMENTS
                                   ...
                                   AF_Dimension   dim )
  {
    ...
    * of the array, used to speed mUp segment linking later...
    */
    {
      AF_Segment  segments = axis->segments;
                                ...
                                AF_Dimension   dim )
  {
     ...
     * segment directly in the code above slows mDown execution speed for
                                     ...
                                     AF_LatinMetrics  metrics )
  {
    ...
    for ( ; edge < edge_limit; edge++ )
    {
      ...
      for ( bb = 0; bb < AF_LATIN_BLUE_MAX; bb++ )
      {
        ...
        /* if it is a mUp zone, check for mRight edges -- if it is a mDown */
        /* zone, check for mLeft edges                                      */
        /*                                                                 */
        /* of course, that's for TrueType                                  */
        is_top_blue  = (FT_Byte)( ( blue->flags & AF_LATIN_BLUE_TOP ) != 0 );
        is_major_dir = FT_BOOL( edge->dir == axis->major_dir );

        /* if it is a mUp zone, the edge must be against the major    */
        /* direction; if it is a mDown zone, it must be in the major */
        /* direction                                                  */
        if ( is_top_blue ^ is_major_dir )
        {
          ...
          /* mUp zone, or under the reference position of a mDown zone   */
                       ...
                       AF_Dimension   dim )
  {
    ...
    for ( edge = edges; edge < edge_limit; edge++ )
    {
      ...
      else
      {
        ...
        AF_LOG(( "ALIGN: mLeft=%.2f mRight=%.2f ", org_left/64.0, org_right/64.0 ));
        ...
        else
        {
          ...
          */
          ...
          if (cur_len <= 96)
          {
            ...
            * when the mLeft and mRight edges of the span each represent
          ...
          /* snapping the mLeft edge to the grid */
          org   = org_left;
          ...
          /* snapping the mRight edge to the grid */
          org   = org_right;
          fit   = FT_PIX_ROUND(org);
          delta = FT_ABS(fit - org);
          displacements[count] = fit - org;
    ...
    /* the third (lowest) stem aligns with the base line; it might end mUp */
