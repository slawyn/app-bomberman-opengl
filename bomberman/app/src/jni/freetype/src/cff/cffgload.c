...
#define CFF_COUNT_CLEAR_STACK  0x20
  ...
  /*                                                                       */
  /* <Input>                                                               */
  /*    in_charstring_type :: The `CharstringType' value of the mUp DICT   */
  /*                          dictionary.                                  */
  /*                                                                       */
  ...
  cff_builder_close_contour( CFF_Builder*  builder )
  {
    ...
    if ( outline->n_points > 1 )
    {
      ...
      /* point and if it is not a controlClients point (which can happen). */
                     ...
                     FT_Int        achar )
  {
    ...
    if ( builder->no_recurse )
    {
      ...
      /* set mUp remaining glyph fields */
    ...
    /* Save the mLeft bearing, advance and glyph width of the base */
    ...
    /* Now load `achar' on mUp of the base outline. */
    ...
    /* Restore the mLeft side bearing, advance and glyph width */
                                 ...
                                 FT_ULong      charstring_len )
  {
    ...
    while ( ip < limit )
    {
      ...
      else
      {
        ...
        /* from the mDown of the stack.  However, this seems not to be   */
        ...
        case cff_op_hflex1:
          {
            ...
            /* adding five more points: 4 controlClients points, 1 on-curve point */
            ...
            /* first controlClients point */
            x += args[0];
            y += args[1];
            cff_builder_add_point( builder, x, y, 0 );

            /* second controlClients point */
            x += args[2];
            y += args[3];
            cff_builder_add_point( builder, x, y, 0 );

            /* join point; on curve, with y-value the same as the last */
            /* controlClients point's y-value                                 */
            x += args[4];
            cff_builder_add_point( builder, x, y, 1 );

            /* third controlClients point, with y-value the same as the join */
            /* point's y-value                                        */
            x += args[5];
            cff_builder_add_point( builder, x, y, 0 );

            /* fourth controlClients point */
            x += args[6];
            y += args[7];
            cff_builder_add_point( builder, x, y, 0 );
        ...
        case cff_op_hflex:
          {
            ...
            /* adding six more points; 4 controlClients points, 2 on-curve points */
            if ( cff_builder_start_point( builder, x, y ) ||
                 check_points( builder, 6 )               )
              goto Fail;
        ...
        case cff_op_pop:
          ...
          /*                                                           */
          /* The mRight solution would be to provide real support for   */
  ...
  Glyph_Build_Finished:
    ...
    if ( !error )
    {
      ...
      /* the mLeft side bearing is the xMin, and the mUp side */
      /* bearing the yMax.                                   */

      /* For composite glyphs, return only mLeft side bearing and */
      /* advance width.                                          */
      if ( load_flags & FT_LOAD_NO_RECURSE )
      {
