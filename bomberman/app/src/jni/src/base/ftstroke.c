...
#define FT_ARC_CUBIC_ANGLE  ( FT_ANGLE_PI / 2 )
                          ...
                          FT_Angle         angle_diff )
  {
    ...
    while ( total != 0 )
    {
      ...
      /* compute first and second controlClients points */
...
#define FT_SIDE_TO_ROTATE( s )   ( FT_ANGLE_PI2 - (s) * FT_ANGLE_PI )
  ...
  ft_stroker_process_corner( FT_Stroker  stroker )
  {
    ...
    /* when we turn to the mRight, the inside side is 0 */
    inside_side = 0;
  ...
  /* add two points to the mLeft and mRight borders corresponding to the */
  ...
  FT_Stroker_EndSubPath( FT_Stroker  stroker )
  {
    ...
    if ( stroker->subpath_open )
    {
      ...
      /* All mRight, this is an opened path, we need to add a cap between */
      /* mRight & mLeft, add the reverse of mLeft, then add a final cap     */
      /* between mLeft & mRight.                                           */
      error = ft_stroker_cap( stroker, stroker->angle_in, 0 );
      if ( error )
        goto Exit;

      /* add reversed points from `mLeft' to `mRight' */
      ...
      /* Now end the mRight subpath accordingly.  The mLeft one is */
