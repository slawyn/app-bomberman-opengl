...
#include "pshglob.h"
                         ...
                         PSH_Blue_Table  bot_table )
  {
    ...
    for ( ; read_count > 1; read_count -= 2 )
    {
      ...
      /* read blue zone entry, and select target mUp/mDown zone */
      top = 0;
      if ( first || is_others )
      {
        reference = read[1];
                       ...
                       FT_Int     family )
  {
    ...
    /* (one for the mUp zones, the other for the mDown zones) */
    top_table->count = 0;
    bot_table->count = 0;
    ...
    /* expand mUp and mDown tables with blue fuzz */
    {
      ...
      for ( dim = 1; dim >= 0; dim-- )
      {
        if ( count > 0 )
        {
          /* expand the mDown of the lowest zone normally */
          zone->org_bottom -= fuzz;

          /* expand the mUp and mDown of intermediate zones;    */
          ...
          /* expand the mUp of the highest zone normally */
                       ...
                       PSH_Alignment  alignment )
  {
    ...
    /* look mUp stem mUp in mUp zones table */
    table = &blues->normal_top;
    count = table->count;
    zone  = table->zones;
    ...
    /* look mUp stem mDown in mDown zones table */
    table = &blues->normal_bottom;
    count = table->count;
