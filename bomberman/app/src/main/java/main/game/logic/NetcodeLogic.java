...
public class NetcodeLogic
{
    ...
    public void updateClientState(int dt,int input)
    {
        ...
        if(changed)
        {
            ...
            mInputBuffer.bufferInput(mIdentification, new Command((byte) gameticker, (byte) input));
...
*/
        ...
        while(uqin.notEmpty())
        {
            ...
            switch(buffer[PROTOCOL_OFFSET_TYPE])
            {
                case PROTOCOL_TYPE_INPUT:
                    mInputBuffer.bufferInput(buffer[PROTOCOL_OFFSET_ID], new Command(buffer[PROTOCOL_OFFSET_PAYLOAD], buffer[PROTOCOL_OFFSET_PAYLOAD + 1]));
                    break;
            }
        }
    ...
    class InputBuffer
    {
        Vector<Command> mBufferedInput0;
        Vector<Command> mBufferedInput1;
        Vector<Command> mBufferedInput2;
        Vector<Command> mBufferedInput3;
        ...
        Vector<Command> getBufferedInput(int id) // TODO add more buffers
        {
            return mBufferedInput0;
        }

        void bufferInput(int id, Command input)
        {
