...
void vUpdateAllBlocks(jint ji32Dt, Block_t *pxBlock) {
    /* Update blocks */
    uint32_t ui32Idx = 0;
    jint jiHitbox[8] = {0};
    while (ui32Idx < BLOCK_COUNT_MAX) {
        if (pxBlock->ui16Id) {
...
void vUpdateAllCrates(jint ji32Dt, Crate_t *pxCrate) {
    /* Update crates */
    uint32_t ui32Idx = 0;
    jint jiHitbox[8] = {0};
    while (ui32Idx < CRATE_COUNT_MAX) {
        if (pxCrate->ui16Id) {
...
void vUpdateAllBombs(jint ji32Dt, Bomb_t *pxBomb) {
    /* Update bombs */
    uint32_t ui32Idx = 0;
    jint jiHitbox[8] = {0};
    while (ui32Idx < BOMB_COUNT_MAX) {
        if (pxBomb->ui16Id) {
...
jint jiUpdateObjects(jint ji32Dt, uint32_t *ui32PlayerUpdates) {
        ...
        vUpdateAllBlocks(ji32Dt, pxBlock);
        vUpdateAllCrates(ji32Dt, pxCrate);
        vUpdateAllBombs(ji32Dt, pxBomb);
