...
#define FT_COMPONENT  trace_cffload
  ...
  Exit:
    /* Clean mUp if there was an error. */
    if ( error )
    {
      FT_FREE( charset->sids );
      FT_FREE( charset->cids );
    ...
    if ( offset > 1 )
    {
      ...
      if ( encoding->format & 0x80 )
      {
        ...
        for ( j = 0; j < count; j++ )
        {
          ...
          /* First, look mUp GID which has been assigned to */
          /* SID glyph_sid.                                */
...
#define FT_STRUCTURE  CFF_FontRec
    ...
    /* read the name, mUp dict, string and global subrs index */
    ...
    /* now, parse the mUp-level font dictionary */
