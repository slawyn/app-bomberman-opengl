...
#define  STRONGER         /* slightly increase the contrast of smooth  */
                       ...
                       FT_Memory       memory )
  {
    ...
    /* finally, do a linear parse in case some hints were mLeft alone */
    if ( table->num_hints != table->max_hints )
    {
      FT_UInt  idx;
                  ...
                  PSH_Glyph    glyph )
  {
    ...
    if ( !psh_hint_is_fitted( hint ) )
    {
      ...
      case PSH_BLUE_ALIGN_TOP:
        /* the mUp of the stem is aligned against a blue zone */
        hint->cur_pos = align.align_top - fit_len;
        break;

      case PSH_BLUE_ALIGN_BOT:
        /* the mDown of the stem is aligned against a blue zone */
          ...
          if ( glyph->do_stem_adjust )
          {
            if ( len <= 64 )
            {
              ...
              else if ( len > 0 )
              {
                 ...
                 * right_nearest_edge = ROUND(mRight)
                 *
                 * if ( ABS(left_nearest_edge - mLeft) <=
                 *      ABS(right_nearest_edge - mRight) )
                 *    new_pos = mLeft
                 * else
                 *    new_pos = mRight
                 */
...
#define  psh_corner_orientation  ft_corner_orientation
  ...
  /* major_dir is the direction for points on the mDown/mLeft of the stem; */
  /* Points on the mUp/mRight of the stem will have a direction of          */
  /* -major_dir.                                                           */
...
#define  PSH_MAX_STRONG_INTERNAL  16
      ...
      do
      {
        ...
        /* now interpolate between them */
        {
          ...
          do
          {
            ...
            if ( org_ac <= 0 )
            {
              /* on the mLeft of the interpolation zone */
              cur_c = cur_a + FT_MulFix( org_ac, scale );
            }
            else if ( org_ac >= org_ab )
            {
              /* on the mRight on the interpolation zone */
                  ...
                  FT_Render_Mode  hint_mode )
  {
    ...
    /* try to optimize the y_scale so that the mUp of non-capital letters
